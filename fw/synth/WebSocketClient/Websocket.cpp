...
int Websocket::send(void * buf, uint32_t len) {
    char msg[len + 15];
    int idx = 0;
    //DBG("Sending Opcode\r\n");
    idx = sendOpcode(0x02, msg);
    //DBG("Send length\r\n");
    idx += sendLength(len, msg + idx);
    //DBG("Send mask\r\n");
    idx += sendMask(msg + idx);
    //DBG("Memcpy\r\n");  
    memcpy(msg+idx, buf, len);
    //DBG("Write\r\n");  
...
int Websocket::read(char * message) {
    int i = 0;
    uint32_t len_msg;
    char opcode = 0;
    char c;
